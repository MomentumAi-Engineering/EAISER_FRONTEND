...
export default function EaiserLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Use unified login helper (routes to localhost:8000 in dev)
      const { login } = await import('../api/auth'); // dynamic import or use existing import
      const data = await login(formData); // Use the helper

      // Store token and user info
      localStorage.setItem('auth_token', data.access_token);
      localStorage.setItem('token', data.access_token);
      localStorage.setItem('user', JSON.stringify(data.user));
      localStorage.setItem('userData', JSON.stringify(data.user)); // Consistent with Signup.jsx
  ...
  const handleGoogleClick = async () => {
    ...
    try {
      ...
      // 1. Try to show the prompt (One Tap)
      googleId.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          console.log("Prompt blocked, rendering standard button into container...");
          // 2. Fallback: Render the official button if prompt fails
        ...
        <div className="relative bg-gradient-to-br from-gray-900/90 to-black/90 backdrop-blur-xl rounded-3xl border border-yellow-500/20 p-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            <div className="relative group overflow-hidden rounded-xl">
              <div ref={googleButtonRef} className="absolute inset-0 z-20 opacity-0 pointer-events-none"></div>
              <button
                type="button"
                onClick={handleGoogleClick}
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 p-3 bg-white hover:bg-gray-100 border border-gray-200 rounded-xl transition-all hover:-translate-y-0.5 disabled:opacity-60 shadow-sm"
              ...
              >
                ...
                <span className="text-black text-xs font-bold">Google</span>
              </button>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="group">
              <label className="block text-xs font-semibold text-gray-400 mb-1.5">Username or Email</label>
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" name="forgot-password" id="forgot-password-link" className="text-xs text-yellow-400 hover:text-yellow-300 font-semibold transition-colors">
                Forgot Password?
              </Link>
            </div>
          ...
          <p className="text-center text-gray-400 text-xs mt-5">
            ...
            <Link to="/signup" state={{ returnTo }} className="text-yellow-400 font-semibold hover:text-yellow-300 transition-colors">
              Sign Up
            </Link>
          </p>
        </div>
