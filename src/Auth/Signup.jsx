...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.detail || err?.response?.message || err.message || 'Signup failed.');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      googleId.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          console.log("Prompt blocked/skipped, rendering standard button into container...");
          // 2. Fallback: Render the official button if the prompt is blocked
          if (googleButtonRef.current) {
            ...
            // Simulate a click on the newly rendered button after a short delay
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="relative bg-gradient-to-br from-gray-900/90 to-black/90 backdrop-blur-xl rounded-3xl border border-yellow-500/20 p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="group">
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 text-gray-500 group-focus-within:text-yellow-400 transition-colors" />
                  <input
                    type="text"
                    name="firstName"
                    ...
                    placeholder="John"
                    className="w-full pl-9 pr-3 py-2 bg-white/5 border border-gray-700 focus:border-yellow-500 rounded-xl text-white text-sm placeholder-gray-500 outline-none transition-all focus:bg-white/10"
                  />
                </div>
              </div>
              {/* Last Name */}
          ...
          <p className="text-center text-gray-400 text-xs mt-4">
            ...
            <Link to="/login" state={{ returnTo }} className="text-yellow-400 font-semibold hover:text-yellow-300 transition-colors">
              Sign In
            </Link>
          </p>
        </div>
