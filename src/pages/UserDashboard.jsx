...
const StatCard = memo(({ icon: Icon, label, value, color, delay, className = "" }) => (
  <motion.div
    initial={{ opacity: 0, y: 10 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.4, delay: delay }}
    className={`relative overflow-hidden rounded-lg sm:rounded-xl bg-gradient-to-br from-zinc-900 to-zinc-800 border border-yellow-500/10 p-2 sm:p-4 cursor-pointer group will-change-transform ${className}`}
  >
    ...
    <div className="relative z-10 flex flex-row items-center justify-start gap-1.5 sm:gap-3">
      <div className={`p-1.5 sm:p-2 rounded-lg ${color} shrink-0`}>
        <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400" />
      </div>

      <div className="text-left min-w-0 flex-1">
        <p className="text-gray-400 text-[8px] sm:text-[10px] md:text-xs font-medium uppercase tracking-wider leading-tight truncate">{label}</p>
        <p className="text-base sm:text-xl md:text-2xl font-bold text-white">{value}</p>
      </div>
    </div>
  </motion.div>
));
                    ...
                    issues
                      ...
                      .map((issue, idx) => (
                        <motion.div
                          ...
                          className="bg-zinc-800/30 border border-white/5 rounded-xl p-3 sm:p-4 hover:border-yellow-500/40 transition-all cursor-pointer group will-change-transform overflow-hidden"
                        >
